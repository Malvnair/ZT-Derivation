...
D = mp.e**(-mp.pi**2 * k_B / (3 * e * 116e-6))  # Empirical D
...
for mu in mu_eV:
    ...
    L_SI = (1.5 + D**R) * 1e-8  # Lorenz number in W·Ω·K⁻²
    sigma = -C * gamma_3_2 * Li_3half  # Electrical conductivity (S/m)
    kappa_e_vals.append(L_SI * abs(sigma) * T)
...
ax.plot(mu_eV, kappa_e_vals, color='#ff7f0e', linewidth=2.5)
ax.set_xlabel('Chemical Potential μ (eV)', fontsize=11)
ax.set_ylabel('κₑ (W/m·K)', fontsize=11)
ax.set_title('Electronic Thermal Conductivity', fontsize=12, fontweight='bold', pad=10)
